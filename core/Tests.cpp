...
static bool ZTT_deepCompareCertificates(const Certificate& a, const Certificate& b)
{
    ...
    for (unsigned int i = 0; i < a.subject.networkCount; ++i) {
        ...
        if (a.subject.networks[i].controller.address != b.subject.networks[i].controller.address)
            return false;
        if (memcmp(
                a.subject.networks[i].controller.hash,
                b.subject.networks[i].controller.hash,
...
extern "C" const char* ZTT_general()
{
    try {
        ...
        if (*(&endian) != 0x0807060504030201ULL) {
            ZT_T_PRINTF(
                "[general] Error: __BYTE_ORDER == __LITTLE_ENDIAN but byte order is actually %.16llx" ZT_EOL_S,
                endian);
        ...
        if (endian != 0x0102030405060708ULL) {
            ZT_T_PRINTF(
                "[general] Error: __BYTE_ORDER == __BIG_ENDIAN but byte order is actually %.16llx" ZT_EOL_S,
                endian);
            return "__BYTE_ORDER incorrectly defined";
        }
#endif
        ...
        Identity v0id, v1id;
        {
            ...
            if (id != id2) {
                ZT_T_PRINTF(
                    "FAILED (v0 unmarshal %s != %s)" ZT_EOL_S,
                    id.toString(true).c_str(),
                    id2.toString(true).c_str());
                return "Identity test failed: v0 unmarshal !=";
            }
...
extern "C" const char* ZTT_crypto()
{
    try {
...

        {
            uint8_t out[16];
            ZT_T_PRINTF(
                "[crypto] Testing AES (hardware acceleration: %s)... ",
                AES::accelerated() ? "enabled" : "disabled");
...

        {
            ...
            uint8_t issuerPrivateKey[ZT_CERTIFICATE_MAX_PRIVATE_KEY_SIZE],
                certPrivateKey[ZT_CERTIFICATE_MAX_PRIVATE_KEY_SIZE];
...
extern "C" const char* ZTT_benchmarkCrypto()
{
    try {
...

        {
            ...
            int64_t t = now() - start;
            ZT_T_PRINTF(
                "%llu ms total, %.8f ms/round, verification: ",
                t,
                (double)t / (double)ZT_IDENTITY_TYPE1_MIMC52_ROUNDS);
            start = now();
...

        {
            ...
            ZT_T_PRINTF(
                " %.4f MiB/sec" ZT_EOL_S,
                ((16384.0 * (double)reps) / 1048576.0) / ((double)(end - start) / 1000.0));
...

        {
            ...
            for (int i = 0; i < 15; ++i) {
                for (int t = 0; t < ZT_NUM_C25519_TEST_VECTORS; ++t) {   // NOLINT(modernize-loop-convert)
                    if (C25519::verify(
                            C25519_TEST_VECTORS[t].pub1,
                            C25519_TEST_VECTORS[t].agreementSha512,
                            64,
                            C25519_TEST_VECTORS[t].agreementSignedBy1,
                            96))
                        ++foo;
                }
            }
