...
To build on Mac and Linux just type `make`. On FreeBSD, MidnightBSD and OpenBSD `gmake` (GNU make) is required and can be installed from packages or ports. For Windows there is a Visual Studio solution in `windows/'.
 ...
 - **MidnightBSD**
   - GNU make is required. Type `gmake` to build.
   - You may install gmake with `mport install gmake`
   - CLANG 6 or better should be used on MidnightBSD.  Install with `mport install llvm60`
 - **OpenBSD**
 ...
 * **FreeBSD** / **MidnightBSD** / **OpenBSD**: `/var/db/zerotier-one`
