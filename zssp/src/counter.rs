...
impl CounterWindow {
    ...
    pub fn message_received(&self, received_counter_value: u32, received_fragment_no: u8) -> bool {
        ...
        for nonce in data.1 {
            ...
            let udist = nonce.abs_diff(fragment_nonce);
            let sdist = (nonce as i64).abs_diff(fragment_nonce as i64);
            if udist < sdist {
                is_gt_min |= nonce < fragment_nonce;
            } else {
                is_gt_min |= (nonce as i64) < (fragment_nonce as i64);
            }
        }
        if !is_in & is_gt_min {
            let idx = data.0;
